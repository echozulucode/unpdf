"""Run all test cases and generate debug output.

This script processes all test PDF files, converts them to Markdown,
generates debug structure files, and calculates accuracy scores.
...
def run_test_with_debug(test_number: int, test_name: str) -> dict:
    """Run a single test case with debug output and return results.
    ...
    debug_file = test_dir / f"{test_number:02d}_debug.txt"
    ...
    # Generate debug structure
    try:
        debug_output = dump_pdf_structure(pdf_file, debug_file)
        print(f"  Debug structure saved to {debug_file}")
    except Exception as e:
        print(f"  Warning: Debug generation failed: {e}")
...
def main():
    ...
    print("=" * 80)
    print("PDF to Markdown Conversion Test Suite (with Debug Output)")
    print("=" * 80)
    print()
    ...
    for test_num, test_name in tests:
        ...
        result = run_test_with_debug(test_num, test_name)
