...
class TableProcessor:
    ...
    def _group_words_into_cells(self, words: list[dict]) -> list[list[dict]]:
        """Group horizontally adjacent words into cells based on spacing.
        
        Words that are close together horizontally belong to the same cell.
        Words with large horizontal gaps belong to different cells.
        
        This solves the problem where multi-word cells like "Header 1" are split
        into separate columns.
        
        Args:
            words: List of word dictionaries from pdfplumber, sorted or unsorted.
            
        Returns:
            List of cell groups, where each group is a list of words in reading order.
            
        Example:
            >>> words = [
            ...     {"text": "Header", "x0": 10, "x1": 40},
            ...     {"text": "1", "x0": 43, "x1": 48},
            ...     {"text": "Header", "x0": 70, "x1": 100},
            ...     {"text": "2", "x0": 103, "x1": 108}
            ... ]
            >>> cells = processor._group_words_into_cells(words)
            >>> [[w["text"] for w in cell] for cell in cells]
            [['Header', '1'], ['Header', '2']]
        """
        if not words:
            return []
        ...
        # Use gap statistics to determine threshold
        sorted_gaps = sorted(gaps)
        median_gap = sorted_gaps[len(sorted_gaps) // 2]
        min_gap = sorted_gaps[0]
        max_gap = sorted_gaps[-1]
        
        # Check if gaps are uniform (all similar) or varied
        gap_range = max_gap - min_gap
        gap_variance_ratio = gap_range / median_gap if median_gap > 0 else 0
        
        if gap_variance_ratio < 0.5:
            # Uniform gaps - likely a table with evenly spaced columns
            # Each word is probably its own cell (unless gaps are very small)
            if median_gap < 10:
                # Very small gaps - words belong together
                gap_threshold = median_gap * 2
            else:
                # Larger gaps - treat each word as separate cell
                # Use a small threshold so most gaps create new cells
                gap_threshold = min(median_gap * 0.8, 15)
        else:
            # Varied gaps - use median-based threshold
            # Heuristic: threshold is 3× median gap
            # This separates small intra-cell gaps from large inter-cell gaps
            gap_threshold = max(median_gap * 3, 10)  # At least 10pt
        
        logger.debug(f"Cell grouping: median_gap={median_gap:.1f}pt, range={gap_range:.1f}pt, "
                    f"variance_ratio={gap_variance_ratio:.2f}, threshold={gap_threshold:.1f}pt")
        
        # Group words into cells based on gaps
        cells = []
        ...
        for i in range(1, len(sorted_words)):
            ...
            if gap < gap_threshold:
                ...
                logger.debug(f"  Same cell: gap={gap:.1f}pt < {gap_threshold:.1f}pt: '{prev_word['text']}' + '{curr_word['text']}'")
            else:
                # Large gap - new cell
                cells.append(current_cell)
                current_cell = [curr_word]
                logger.debug(f"  New cell: gap={gap:.1f}pt >= {gap_threshold:.1f}pt: '{prev_word['text']}' | '{curr_word['text']}'")
        
        # Don't forget the last cell
        if current_cell:
            cells.append(current_cell)
        
        logger.debug(f"Grouped {len(sorted_words)} words into {len(cells)} cells")
    ...
    def _build_table_candidate(self, sorted_rows: list[tuple], start_idx: int) -> list[dict] | None:
        ...
        # Group words into cells first
        first_row_cells = self._group_words_into_cells(words_start)
        
        # Detect column structure from cells (not individual words)
        columns = self._detect_column_positions_from_cells(first_row_cells)
        ...
        for i in range(start_idx + 1, len(sorted_rows)):
            ...
            row_cells = self._group_words_into_cells(words)
            row_columns = self._detect_column_positions_from_cells(row_cells)
    ...
    def _detect_column_positions_from_cells(self, cells: list[list[dict]]) -> list[float]:
        """Detect column x-positions from cell groups.
        
        Uses the center position of each cell to determine column locations.
        
        Args:
            cells: List of cell groups (each cell is a list of words).
        ...
        Example:
            >>> cells = [
            ...     [{"x0": 10, "x1": 40}, {"x0": 43, "x1": 48}],  # "Header 1"
            ...     [{"x0": 70, "x1": 100}, {"x0": 103, "x1": 108}]  # "Header 2"
            ... ]
            >>> positions = processor._detect_column_positions_from_cells(cells)
            >>> # Returns approximate centers of each cell group
        """
        if not cells:
            return []
        ...
        for cell in cells:
            ...
            # Calculate cell center x-position
            cell_x0 = cell[0]['x0']
            cell_x1 = cell[-1]['x1']
            cell_center = (cell_x0 + cell_x1) / 2
            
            column_positions.append(cell_center)
        
        return column_positions
    
    def _columns_match(self, cols1: list[float], cols2: list[float], tolerance: float = 25.0) -> bool:
        """Check if two column structures match.
        
        Args:
            cols1: First column x-positions.
            cols2: Second column x-positions.
            tolerance: Maximum difference in pixels. Default 25.0 for cell-center based detection.
    ...
    def _calculate_table_confidence_v2(self, candidate_rows: list[dict], page: Any) -> float:
        ...
        # NOTE: When using cell centers (not word starts), alignment may be less precise
        max_variance = 0.0
        ...
        # More lenient thresholds for cell-based detection
        if max_variance < 5:
            confidence += 0.4
        elif max_variance < 15:
            confidence += 0.3
        elif max_variance < 25:
            confidence += 0.2
    ...
    def _extract_table_from_candidate_v2(self, candidate_rows: list[dict]) -> list[list[str]]:
        """Extract table data from candidate rows using cell grouping.
        
        This version groups words into cells first (e.g., "Header" + "1" = "Header 1"),
        then assigns complete cells to columns. This prevents multi-word cells from
        being split across columns.
        
        Args:
            candidate_rows: List of row dictionaries with 'cells' and 'columns' keys.
        ...
        Example:
            >>> row = {
            ...     'cells': [[{'text': 'Header'}, {'text': '1'}], [{'text': 'Header'}, {'text': '2'}]],
            ...     'columns': [25, 75]
            ... }
            >>> result = processor._extract_table_from_candidate_v2([row])
            >>> result[0]
            ['Header 1', 'Header 2']
        """
        # First, determine the unified column positions from all rows
        # Use the first row as reference
        if not candidate_rows:
            return []
        
        num_columns = len(candidate_rows[0].get('cells', []))
        
        # For each row, assign cells to columns
        table_data = []
        ...
        for row in candidate_rows:
            ...
            if not row_cells:
                # Fallback to old method if cells not available
                logger.warning("Row missing 'cells' field, using fallback")
                ...
                # New method: cells are already grouped
                # Convert cell groups to strings
                row_data = []
                for cell_group in row_cells:
                    # Join words within the cell
                    cell_text = ' '.join(word['text'] for word in cell_group)
